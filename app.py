...
print(f"[INFO] Archivos detectados en {base}:")
for f in base.glob("*"):
    print(" -", f.name)
...
try:
    ...
    print("[ERROR] No se pudo leer Datos_2023_550.xlsx:", e)
    mm = pd.DataFrame()

try:
    deptos = gpd.read_file(path_shp).to_crs(4326)
except Exception as e:
    print("[ERROR] No se pudo leer shapefile:", e)
    deptos = gpd.GeoDataFrame()

try:
    nv = pd.read_excel(path_nv, engine="openpyxl")
except Exception as e:
    print("[ERROR] No se pudo leer Nacimientos_2023.xlsx:", e)
    nv = pd.DataFrame()

if deptos.empty or mm.empty or nv.empty:
    print("[CRITICAL] Archivos no cargados correctamente. Mostrando página vacía.")
...
# === PROCESAMIENTO (solo si los datos existen) ===
if not deptos.empty and not mm.empty and not nv.empty:
...
def actualizar_mapas(clasif):
    if datos.empty or gjson_global is None:
        print("[WARNING] Datos vacíos: Render mostrando fallback.")
        fig_empty = px.scatter(title="No hay datos disponibles para mostrar.")
    ...
    fig_rmm = px.choropleth_mapbox(
        df, geojson=gjson, locations="DEPTO_CODE",
        featureidkey="properties.DEPTO_CODE", color="RMM_100k",
        hover_name="DEPTO_NOMBRE", color_continuous_scale="YlOrRd",
        mapbox_style="carto-positron", zoom=4.3, center={"lat": 4.5, "lon": -74},
    ...
    fig_mm = px.choropleth_mapbox(
        df, geojson=gjson, locations="DEPTO_CODE",
        featureidkey="properties.DEPTO_CODE", color="MM_MUERTES",
        hover_name="DEPTO_NOMBRE", color_continuous_scale="Oranges",
        mapbox_style="carto-positron", zoom=4.3, center={"lat": 4.5, "lon": -74},
    ...
    for fig in [fig_rmm, fig_mm]:
        fig.update_layout(
            ...
            coloraxis_colorbar=dict(title="", tickfont=dict(size=12),
