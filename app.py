...
deptos["geometry"] = deptos["geometry"].simplify(tolerance=0.01, preserve_topology=True)
    ...
    "features": [
        {
            ...
            "properties": {"DEPTO_CODE": row.DEPTO_CODE}
        }
        for _, (geom, row) in enumerate(zip(datos.geometry, datos.itertuples()))
        if geom is not None
    ]
}
gjson_global = geojson_light
